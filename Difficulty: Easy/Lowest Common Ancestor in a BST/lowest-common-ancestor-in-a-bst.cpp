...
class Solution {
  public:
    Node* LCA(Node* root, Node* n1, Node* n2) {
        while (root != nullptr) {
            // If both n1 and n2 are smaller than root->data,
            // then LCA lies in the left subtree.
            if (n1->data < root->data && n2->data < root->data)
                root = root->left;
            // If both n1 and n2 are greater than root->data,
            // then LCA lies in the right subtree.
            else if (n1->data > root->data && n2->data > root->data)
                root = root->right;
            // Else, we've found the split point which is the LCA.
            else
                return root;
        }
        return nullptr; // Should not be reached if both n1 and n2 exist in the BST.
    }
};
